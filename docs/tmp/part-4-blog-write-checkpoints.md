# Offline-First KMP · The optimistic write you never clean up

*Provisional state should be disposable: let the sync layer reconcile it, don't write cleanup.*
*(Told through PowerSync write checkpoints.)*

> Part 4 of the **Offline-First KMP** series: field notes from a production winery app built on Kotlin
> Multiplatform. Part 2 established the
> [two-write model](./part-2-blog-two-writes-for-every-tap.md) (an uploaded event plus a local-only
> projection) and made a promise I didn't cash: your provisional local row gets replaced by the
...
PowerSync's *write checkpoints* hold incoming server state until your pending uploads are accounted for,
...
## The Claim That Sounds Like a Lie

Here's the two-write model from Part 2, on a fresh example. You perform an action offline, such as
...
client already minted for it (Part 3), and syncs it back down.
...
## What I Got Wrong Reading the Logs
...
gates: pending local writes → hold the checkpoint → retry once the queue is drained. That log line I
panicked over is the gate doing its job.
...
It isn't. What reverts is the projection, never the event: the event is the immutable record from Part 2,
and it left the device via the upload queue before the sweep ever runs. The sweep removes a disposable
guess; the durable fact was never stored in that row to begin with.

## Why the ID Has to Match

This is where Part 3 pays off. If your local projection and the server's authoritative row had
*different* ids, the reconcile wouldn't be "one row out, one row in": it'd be your row removed and a
*different-looking* row added, and for a flicker the UI would lose the entity entirely, or worse, briefly
show two.
...
and Part 3 covered why that's safe to arrange.)

## The One Mechanism to Internalize

The whole local-only-projection model (instant UI, no cleanup code) rests on a single mechanism:
**PowerSync gates downloaded checkpoints behind a drained upload queue, then reconciles the local DB to
server truth, dropping any local row that isn't in a server bucket.**
...
That's the mechanism under the "no cleanup code of ours" promise. In the next parts I'll switch from
